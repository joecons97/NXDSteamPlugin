...
namespace NXDSteamPlugin.Services.GameDetection
{
    public class SteamLocalService
    {
        ...
        public static HashSet<string> GetLibraryFolders()
        {
            ...
            try
            {
                using (var fs = new FileStream(configPath, FileMode.Open, FileAccess.Read, FileShare.ReadWrite))
                {
                    var kv = new KeyValue();
                    kv.ReadAsText(fs);
                    foreach (var dir in GetLibraryFolders(kv).Where(Directory.Exists))
                    {
                        dbs.Add(dir);
                    }
                }
        ...
        public static UniTask<List<LibraryEntry>> GetInstalledGamesAsync(CancellationToken cancellationToken)
        {
            ...
            return UniTask.FromResult(games
                .Select(x => x.Value)
                .ToList());
        }
